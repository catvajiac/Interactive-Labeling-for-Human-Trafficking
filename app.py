...
def gen_page_content(state, df):
    ...
    header_stats, micro_cluster_features, timeline_features = utils.feature_extract(subdf)
    ...
    with mid_col:
        ...
        st.altair_chart(draw.stream_chart(timeline_features), use_container_width=True)

    # show map of ad locations
    with right_col:
        st.header('**Geographical spread of ads**: select a range of dates')
        date_range = pd.date_range(min(subdf.days), max(subdf.days)).strftime(utils.DATE_FORMAT)
        dates = st.select_slider('', options=date_range, value=(date_range[0], date_range[-1]))
        st.write(draw.map(subdf[['ad_id', 'lat', 'lon', 'location', 'days']], dates))
